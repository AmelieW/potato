...
def run_in_pipeline(directory, to_be_labeled):
    """starts the tracking pipeline"""

    all_annotations = []

    print(f"Reading in data from {directory}")
    ...
    L = [l for l in L if (len(l) == 24) or (l == 'debug_user')]

    for prolific_id in L:
        file = f"{directory}{prolific_id}/annotated_instances.jsonl"
    ...
    stats, productive_annotators = instance_stats(all_annotations)
    print(type(stats))
    ...
    instances_to_be_labeled = pd.read_csv(to_be_labeled, sep="\t")
    ...
    return stats, productive_annotators
